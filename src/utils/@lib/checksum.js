...
}), k = c.enc = {}, m = k.Hex = { stringify: function (a) { var d = a.words; a = a.sigBytes; for (var b = [], e = 0; e < a; e++) { var c = d[e >>> 2] >>> 24 - 8 * (e % 4) & 255; b.push((c >>> 4).toString(16)); b.push((c & 15).toString(16)) } return b.join("") }, parse: function (a) {
...
}); b.Hasher = x.extend({ cfg: l.extend(), init: function (a) { this.cfg = this.cfg.extend(a); this.reset() }, reset: function () { x.reset.call(this); this._doReset() }, update: function (a) { this._append(a); this._process(); return this }, finalize: function (a) { a && this._append(a); return this._doFinalize() }, blockSize: 16, _createHelper: function (a) { return function (b, c) { return (new a.init(c)).finalize(b) } }, _createHmacHelper: function (a) {
    return function (b, c) {
...
(function () {
    var a = CryptoJS, j = a.enc.Utf8; a.algo.HMAC = a.lib.Base.extend({ init: function (a, b) { a = this._hasher = new a.init; "string" == typeof b && (b = j.parse(b)); var f = a.blockSize, l = 4 * f; b.sigBytes > l && (b = a.finalize(b)); b.clamp(); for (var u = this._oKey = b.clone(), k = this._iKey = b.clone(), m = u.words, y = k.words, z = 0; z < f; z++) m[z] ^= 1549556828, y[z] ^= 909522486; u.sigBytes = k.sigBytes = l; this.reset() }, reset: function () { var a = this._hasher; a.reset(); a.update(this._iKey) }, update: function (a) { this._hasher.update(a); return this }, finalize: function (a) {
        var b =
...
function fnAddDays(date, days) {
    var result = (typeof OriginalDate !== 'undefined') ? new OriginalDate() : new Date();
    result.setDate(result.getDate() + days);
    return result;
}

export function fnCheckSum(data) {
    var date = (typeof OriginalDate !== 'undefined') ? new OriginalDate() : new Date();
    var weekday1 = (date.getDay() + 1).toString()
    var yearStr1 = date.getFullYear().toString()
    var sYear1 = yearStr1.substr(0, 2)
    var eYear1 = yearStr1.substr(2, 2)
    var day1 = date.getDate().toString()
    var hour1 = date.getHours()
    var saltKey1 = weekday1 + sYear1 + day1 + eYear1
    date = fnAddDays(date, -1)

    var weekday2 = (date.getDay() + 1).toString()
    var yearStr2 = date.getFullYear().toString()
    var sYear2 = yearStr2.substr(0, 2)
    var eYear2 = yearStr2.substr(2, 2)
    var day2 = date.getDate().toString()
    var saltKey2 = weekday2 + sYear2 + day2 + eYear2
	if (hour1 < 6) {
		return CryptoJS.HmacSHA512(JSON.stringify(data), saltKey2).toString()
		
	} else {
		return CryptoJS.HmacSHA512(JSON.stringify(data), saltKey1).toString()
		console.log('saltKey1 ==>',saltKey1)
	}
}
