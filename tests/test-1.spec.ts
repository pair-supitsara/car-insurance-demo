...
test('test', async ({ page }) => {
  ...
  await page.getByRole('combobox').nth(1).selectOption('TOYOTA');
  await page.getByRole('combobox').nth(2).selectOption('CAMRY');
  await page.getByRole('combobox').nth(3).selectOption('HEV SMART / Auto / 2.5 CC / 4 ประตู');
  await page.getByRole('radio', { name: 'ติด', exact: true }).check();
  await page.getByRole('radio', { name: 'ไม่ให้ นิติบุคคล' }).check();
  await page.getByRole('combobox').nth(4).selectOption('16');
  await page.locator('#rd_give_register').nth(1).check();
  await page.getByRole('combobox').nth(5).selectOption('12');
  await page.getByRole('spinbutton', { name: 'จำนวนวันที่ใช้รถต่อสัปดาห์ ปีนี้สนใจประกันประเภทไหนคะ' }).dblclick();
  await page.getByRole('spinbutton', { name: 'จำนวนวันที่ใช้รถต่อสัปดาห์ ปีนี้สนใจประกันประเภทไหนคะ' }).fill('2');
